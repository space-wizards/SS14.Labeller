...
public class GitHubApiClient(HttpClient httpClient) : IGitHubApiClient
{
    ...
    /// <inheritdoc />
    public async Task<string> CreateIssue(string issueTitle, string issueBody, GithubRepo target, CancellationToken ct)
    {
        ...
        var json = JsonSerializer.Serialize(request, SourceGenerationContext.Default.CreateIssueRequest);
        var content = new StringContent(json, Encoding.UTF8, "application/json");

        var issueRes = await httpClient.PostAsync($"{BaseUrl}/repos/{target.Owner}/{target.Name}/issues", content, ct);
        issueRes.EnsureSuccessStatusCode();

        var issueJson = JsonDocument.Parse(await issueRes.Content.ReadAsStringAsync(ct));
        return issueJson.RootElement.GetProperty("html_url").GetString()!;
    }
